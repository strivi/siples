	...
	if (! empty ( $grid ['href'] )) {
		...
		foreach ( $links as $link ) {
			...
			if (preg_match ( '/^\[([a-z_]+)\]$/', $href, $matches )) {
				...
				if($show[0] == '?') {
                	$show = substr($show, 1);
                	$confirm = ' onclick="if(!confirm(\'你确信要执行此操作？\')) {return false;}"';
                } else {
                	$confirm = '';
                }
								
                $val[]	=	'<a target="' . $target . '" href="'.urldecode ( U ( $href ) ).'"'.$confirm.'>'.$show.'</a>';
                /*
				if ($show == '删除') {
...
// 获取api_ticket，自动带缓存功能
function get_api_ticket($type, $access_token = '') {
	empty ( $access_token ) && $access_token = get_access_token ();
	if(empty ( $access_token )) {
		return 0;
	}
	$key = $type.'_ticket_' . $access_token;
	$res = S ( $key );
	if ($res !== false)
		return $res;
 ...
 * @return int 参与人数
 */	
function get_part_num($target_id){
	$map['target_id'] = $target_id;
	$part_num = M('ScratchMember')->where($map)->count();
	return $part_num;
 ...
 * @return int 该活动的刮奖次数
 */	
function get_scratch_num($target_id){
	$map['target_id'] = $target_id;
	$scratch_num = M('SnCode')->where($map)->count();
	return $scratch_num;
}  
 ...
 * @return int 该活动的刮奖次数
 */	
function get_share_num($target_id){
	$map['target_id'] = $target_id;
	$map['share'] = 1;
	$share_num = M('ScratchMember')->where($map)->count();
	return $share_num;
}  
 ...
 * @return int 该活动的刮奖次数
 */	
function set_sence_scratch($dataArr){
		$map_follow['openid'] = $dataArr['FromUserName'];
		$follow = M('Follow')->where($map_follow)->find();
		//注意屏蔽已经关注过的人
		if(!$follow){
			$map['id'] = (int)(substr($dataArr['EventKey'], 8)/100000);
			M('ScratchMember')->where($map)->setInc('subscribeNum');
			//$num = M('ScratchMember')->where($map)->getField('subscribeNum');
			//SCRATCH_NUM_PER全局变量，在CONFIG文件中更改
			//$data['scratchnum'] = (int)($num/C('SCRATCH_NUM_PER'));
			//M('ScratchMember')->where($map)->save($data);
			$return = TRUE;
		}else
			$return  = FALSE;
		return $return;
 ...
 * 活动场景数据修改
 *
 * @param $dataArr 关注时由腾讯传入的参数
 */	

function  set_follow_info($dataArr){
	require_once ONETHINK_ADDON_PATH . 'UserCenter/Model/WeixinAddonModel.class.php';
	$model = D ( 'Addons://UserCenter/WeixinAddon' );
	! method_exists ( $model, 'subscribe' ) || $model->subscribe ( $dataArr );
}

/**
 * 根据EventKey获取到ExtraInt
 *
 * @param $EventKey 
 */	
function getExtraIntByEventKey($EventKey){
	$map['scene_id'] = $EventKey;
	$ExtraInt = M('QrCode')->where($map)->getField('extra_int');
	return $ExtraInt;
}
 ...
 * @param $EventKey 
 */	
function getQrCodeByEventKey($EventKey){
	$map['scene_id'] = $EventKey;
	$OrCode = M('QrCode')->where($map)->find();
	return $OrCode;
}

/**
 * 导出微邀请签到列表
 * @param  array $excel_data 微邀请签到数据
 * @param  string $excel_name Excel表名
 */
function excel_set($excel_data,$excel_name){ 
	...
	//set font size bold  
	$objPHPExcel->getActiveSheet()->getDefaultStyle('A1')->getFont()->setSize(14);  
	$objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setBold(true); 

	$objPHPExcel->getActiveSheet()->getDefaultStyle('A2:F2')->getFont()->setSize(12);  
	...
	$objPHPExcel->setActiveSheetIndex(0)  
		->setCellValue('A1', $excel_name.'签到表')
		->setCellValue('A2', 'ID')  
		->setCellValue('B2', '邀请名')  
		->setCellValue('C2', '姓名')  
		->setCellValue('D2', '是否参加')  
		->setCellValue('E2', '联系电话')  
		->setCellValue('F2', '签到内容');  

	// Miscellaneous glyphs, UTF-8  
	for($i=0;$i<count($excel_data);$i++){  
		$objPHPExcel->getActiveSheet(0)->setCellValue('A'.($i+3), ($i+1));  
		$objPHPExcel->getActiveSheet(0)->setCellValue('B'.($i+3), $excel_data[$i]['mid']);  
		$objPHPExcel->getActiveSheet(0)->setCellValue('C'.($i+3), $excel_data[$i]['username']);  
		$objPHPExcel->getActiveSheet(0)->setCellValue('D'.($i+3), $excel_data[$i]['rdogo']);  
		$objPHPExcel->getActiveSheet(0)->setCellValue('E'.($i+3), $excel_data[$i]['telphone']);  
		$objPHPExcel->getActiveSheet(0)->setCellValue('F'.($i+3), $excel_data[$i]['content']);
	...
	header('Content-Disposition: attachment;filename="'.$excel_name.'签到表('.date('Ymd-H:i:s').').xls"');  
...
//获取绿云会员信息
function  getMemberIHotel(){
	return $_SESSION['weiphp_home']['member'];	
	
}

//根据参数名返回对应的数组
function getArrayByPart($array,$param = NULL){	
	$arrayRetrun = array();
	if(!$param)
		$this->error('参数传入错误');
		
	foreach($array as $value){
		$arrayRetrun[$value[$param]] = $value;
	}
	return $arrayRetrun;
}

function getInnNameByInnId($innId){
	$map['id'] = $innId;
	$innName = M('Inn')->where($map)->getField('BranchName');
	return $innName;
}
