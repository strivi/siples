...
* 微信红包支付接口
*/
class SendRedpack_pub extends Common_util_pub{
	var $parameters;//请求参数，类型为关联数组
	public $response;//微信返回的响应
	public $result;//返回参数，类型为关联数组
	var $url ;//接口链接
	...
	function createXml()
	{
	   	...
	   	$this->parameters["mch_id"] = $this->mchid;//商户号
	   	//$this->parameters["sub_mch_id"] = $this->mchid;//子商户号
	    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
	    $this->parameters["re_openid"] = get_openid();
	    $this->parameters["client_ip"] = get_client_ip();
	    $this->parameters["sign"] = $this->getSign($this->parameters);//签名
	    return  $this->arrayToXml($this->parameters);
	}
	 ...
	 * 	作用：使用证书post请求xml
	 */
	function postXmlSSLCurl( $strXml,$url,$second=30) {
		...
		curl_setopt($ch, CURLOPT_SSLCERT,'/alidata/www/default/onesword/Addons/Payment/Controller/WeixinpayV3/cacert//apiclient_cert.pem');
		curl_setopt($ch, CURLOPT_SSLKEY,'/alidata/www/default/onesword/Addons/Payment/Controller/WeixinpayV3/cacert//apiclient_key.pem');
		// CA根证书（用来验证的网站证书是否是CA颁布）
		curl_setopt($ch, CURLOPT_CAINFO, '/alidata/www/default/onesword/Addons/Payment/Controller/WeixinpayV3/cacert/rootca.pem'); 
...
* 微信现金抵用券接口
*/
class SendCoupon_pub extends Common_util_pub{
	var $parameters;//请求参数，类型为关联数组
	public $response;//微信返回的响应
	public $result;//返回参数，类型为关联数组
	var $url ;//接口链接
	...
	function createXml()
	{
	   	...
	   	//$this->parameters["sub_mch_id"] = $this->mchid;//子商户号
	    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
	    //$this->parameters["openid"] = 'o3ZYauOnS_g-qQ9bYISisG2MLfvE';
	    $this->parameters["openid"] = get_openid();
	    $this->parameters["sign"] = $this->getSign($this->parameters);//签名
	    return  $this->arrayToXml($this->parameters);
	}
