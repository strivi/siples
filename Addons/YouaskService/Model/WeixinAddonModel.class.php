...
class WeixinAddonModel extends WeixinModel{
	function reply($dataArr, $keywordArr = array()) {
		...
		$user_status = S ( 'user_status_' . $dataArr['FromUserName'] );
		if($user_status){
			$keywordArr = $user_status['keywordArr'];
		}			
		$content = $keywordArr["keyword"];
		//$this->replyText(json_encode($dataArr));
		//判断微信用户发送的消息中的关键词
		$token = get_token();	
		//addWeixinLog ("进入", 0);
		//直接进入人工操作
		...
		//$this->replyText(json_encode($keywordsValue,JSON_UNESCAPED_UNICODE));	

		//查询匹配方式
		foreach($keywordsValue as $v){
			$ispass  =false;
			//多项匹配,只取第一项
			switch($v["msgkeyword_type"]){//完全匹配/左边匹配/右边匹配/模糊匹配				
				case "0":
